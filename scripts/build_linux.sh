...
usage() {
  ...
  echo "  --dev              Enable tests + samples (VNE_XR_DEV/TESTS/SAMPLES=ON)"
  echo "  --with-openxr      Build OpenXR backend + Vulkan (Monado / PC VR on Linux)"
  echo "  --with-tests       Build vnexr_tests"
  echo "  --with-examples    Build sample apps (alias: --with-samples; 02_openxr_stereo when --with-openxr)"
  echo "  --no-tests         Omit tests"
  echo "  --no-examples      Omit samples (alias: --no-samples)"
  echo ""
